...
class MyTestCase_CLJK_DLWL_Edit(unittest.TestCase):
    '''车辆监控平台-地理围栏模块-编辑测试集'''
    ...
    def tearDownClass(cls):
        ...
        #     'C:\\Users\starlinkware\PycharmProjects\\CK_CLJK\SBZLGL\image\DLWL_Edit.png')
        # 关闭浏览器
        cls.dr.quit()

    # @unittest.skip('跳过用例')
    # 装饰器，当你没有报错也要截图的话，那么你需要在用例里面调用save_img('001')方法
    @BeautifulReport.add_test_img('test_CLJK_DLWL_Edit1')
    def test_CLJK_DLWL_Edit1(self):
        '''地理围栏模块-编辑测试用例-修改后'''
        # 点击车辆监控中心
        ...
        self.dr.find_element_by_xpath(
            '//input[@placeholder="可输入围栏名称进行搜索"]').send_keys('测试围栏20-12-1-1')
        time.sleep(1)
        # 点击查询按钮
        self.dr.find_element_by_xpath('//*[@id="pane-second"]/div[1]/div[1]/div').click()
        time.sleep(4)
        # 点击编辑按钮
        self.dr.find_element_by_xpath('//i[@title="编辑围栏"]').click()
        time.sleep(2)
        # 定位城市
        self.dr.find_element_by_xpath(
            '//*[@id="pane-second"]/div[2]/div/input').send_keys('硚口区')
        time.sleep(1)
        # 点击查询按钮
        self.dr.find_element_by_xpath('//*[@id="pane-second"]/div[2]/div/div/button').click()
        time.sleep(2)
        # 修改围栏名称
        self.dr.find_element_by_xpath(
            '//*[@id="pane-second"]/div[2]/form/div[1]/div/div/input').clear()
        self.dr.find_element_by_xpath(
            '//*[@id="pane-second"]/div[2]/form/div[1]/div/div/input').send_keys('测试围栏12-1-11')
        time.sleep(1)
        # 选择画圆
        self.dr.find_element_by_xpath(
        ...
        # 点击提交
        self.dr.find_element_by_xpath(
            '//*[@id="pane-second"]/div[2]/form/div[2]/div/button[2]').click()
        time.sleep(2)
        # 获取页面元素，保存到变量
        p = self.dr.find_element_by_xpath('//div[@role="alert"]/p').text
        # 判断预期结果与实际结果是否一致
        self.assertEqual(p, '操作成功', '编辑围栏失败')
        time.sleep(4)

    # @unittest.skip('跳过用例')
    # 装饰器，当你没有报错也要截图的话，那么你需要在用例里面调用save_img('001')方法
    @BeautifulReport.add_test_img('test_CLJK_DLWL_Edit2')
    def test_CLJK_DLWL_Edit2(self):
        '''地理围栏模块-编辑测试用例-修改前'''
        # 点击车辆监控中心
        self.dr.find_element_by_id('ztree_1_span').click()
        time.sleep(6)
        ...
        # 输入围栏名称
        self.dr.find_element_by_xpath(
            '//input[@placeholder="可输入围栏名称进行搜索"]').send_keys('测试围栏12-1-11')
        time.sleep(1)
        # 点击查询按钮
        self.dr.find_element_by_xpath('//*[@id="pane-second"]/div[1]/div[1]/div').click()
        time.sleep(4)
        ...
        self.dr.find_element_by_xpath(
            '//*[@id="pane-second"]/div[2]/form/div[1]/div/div/input').clear()
        self.dr.find_element_by_xpath(
            '//*[@id="pane-second"]/div[2]/form/div[1]/div/div/input').send_keys('测试围栏12-1-1')
        time.sleep(1)
        # 选择画圆
        self.dr.find_element_by_xpath(
            '//*[@id="monitorMap"]/div[2]/div/div/div[1]/div/label[1]/span[1]/span').click()
...
if __name__ == '__main__':
    ...
    # 添加测试用例
    testunit.addTest(MyTestCase_CLJK_DLWL_Edit('test_CLJK_DLWL_Edit1'))
    testunit.addTest(MyTestCase_CLJK_DLWL_Edit('test_CLJK_DLWL_Edit2'))
    # 定义测试报告
    runner = BeautifulReport(testunit)
    runner.report(filename='车辆监控平台-地理围栏模块-编辑测试报告',
