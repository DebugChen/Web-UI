...
class MyTestCase_CLJK_PCZHGL_Logout(unittest.TestCase):
    ...
    @BeautifulReport.add_test_img('test_CLJK_PCZHGL_Logout1')
    def test_CLJK_PCZHGL_Logout1(self):
        '''PC账户管理模块-注销测试用例-注销'''
        # 输入用户账户
        ...
        # 点击注销
        self.dr.find_element_by_xpath(
            '//table[@class="el-table__body"]/tbody/tr[1]/td[6]/div/button[4]').click()
        time.sleep(2)
        # 点击确定
        self.dr.find_element_by_xpath(
        ...
        # 判断预期结果与实际结果是否一致
        self.assertEqual(span, '注销', '注销PC账户失败')
        time.sleep(1)
        # 点击清除
    ...
    # @unittest.skip('跳过用例')
    # 装饰器，当你没有报错也要截图的话，那么你需要在用例里面调用save_img('001')方法
    @BeautifulReport.add_test_img('test_CLJK_PCZHGL_Logout2')
    def test_CLJK_PCZHGL_Logout2(self):
        '''PC账户管理模块-注销测试用例-解除注销'''
        # 输入用户账户
        self.dr.find_element_by_xpath(
            '//input[@placeholder="请输入需要搜索的用户账号"]').send_keys('CSZH002')
        time.sleep(1)
        ...
        span = self.dr.find_element_by_xpath(
            '//table[@class="el-table__body"]/tbody/tr[1]/td[5]/div/span').text
        # 判断预期结果与实际结果是否一致
        self.assertEqual(span, '正常', '解除注销PC账户失败')
        time.sleep(2)
        # 点击清除
        self.dr.find_element_by_xpath('//button[@title="清除所有条件"]').click()
        time.sleep(3)
...
if __name__ == '__main__':
    ...
    # 添加测试用例
    testunit.addTest(MyTestCase_CLJK_PCZHGL_Logout('test_CLJK_PCZHGL_Logout1'))
    testunit.addTest(MyTestCase_CLJK_PCZHGL_Logout('test_CLJK_PCZHGL_Logout2'))
    # 定义测试报告
