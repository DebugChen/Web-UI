...
class MyTestCase_CLJK_PCZHGL_Edit(unittest.TestCase):
    ...
    def test_CLJK_PCZHGL_Edit1(self):
        ...
        # 点击编辑
        self.dr.find_element_by_xpath(
            '//table[@class="el-table__body"]/tbody/tr[1]/td[6]/div/button[3]').click()
        time.sleep(2)
        # 修改用户账户
        self.dr.find_element_by_xpath('//input[@placeholder="请输入用户账号"]').clear()
        self.dr.find_element_by_xpath('//input[@placeholder="请输入用户账号"]').send_keys('CSZH0022')
        time.sleep(2)
        # 修改用户名
        self.dr.find_element_by_xpath('//input[@placeholder="请输入用户名"]').clear()
        self.dr.find_element_by_xpath('//input[@placeholder="请输入用户名"]').send_keys('CSZH0022')
        time.sleep(2)
        # 修改手机号
        ...
        # 选择状态：锁定
        self.dr.find_element_by_xpath(
            '//form[@class="el-form"]/div[8]/div/label[2]/span[1]/span').click()
        time.sleep(2)
        # 选择地图展示位置：湖北省
        ...
        self.dr.find_element_by_xpath('//form[@class="el-form"]/div[12]/div/div/div[1]/i').click()
    ...
    def test_CLJK_PCZHGL_Edit2(self):
        ...
        # 输入用户账户
        self.dr.find_element_by_xpath(
            '//input[@placeholder="请输入需要搜索的用户账号"]').send_keys('CSZH0022')
        time.sleep(1)
        # 点击查询
        self.dr.find_element_by_xpath('//button[@title="搜索"]').click()
        time.sleep(2)
        ...
        # 修改用户账户
        self.dr.find_element_by_xpath('//input[@placeholder="请输入用户账号"]').clear()
        self.dr.find_element_by_xpath('//input[@placeholder="请输入用户账号"]').send_keys('CSZH002')
        time.sleep(2)
        # 修改用户名
        self.dr.find_element_by_xpath('//input[@placeholder="请输入用户名"]').clear()
        ...
        # 选择状态：正常
        self.dr.find_element_by_xpath(
            '//form[@class="el-form"]/div[8]/div/label[1]/span[1]/span').click()
        time.sleep(2)
        # 选择地图展示位置：湖北省
        self.dr.find_element_by_xpath('//input[@placeholder="请选择您的地址"]').click()
        self.dr.find_element_by_xpath('//input[@placeholder="请选择您的地址"]').send_keys(Keys.DOWN)
        ...
        # 选择管辖客户：车辆监控中心
        self.dr.find_element_by_xpath('//input[@id="searchData2"]').clear()
        self.dr.find_element_by_xpath('//input[@id="searchData2"]').send_keys('车辆监控中心')
        self.dr.find_element_by_xpath('//form[@class="el-form"]/div[12]/div/div/div[1]/i').click()
        self.dr.find_element_by_xpath('//*[@id="ztree2_1_span"]').click()
        time.sleep(1)
        # 点击提交
        self.dr.find_element_by_xpath(
            '//*[@id="app"]/div/div[6]/div[3]/div/div[3]/div/button[1]').click()
        ...
        div = self.dr.find_element_by_xpath(
            '//table[@class="el-table__body"]/tbody/tr[1]/td[2]/div').text
        # 判断预期结果与实际结果是否一致
        self.assertEqual(div, 'CSZH002', '编辑PC账户失败')
        # 获取页面元素，保存到变量
        span = self.dr.find_element_by_xpath(
            '//table[@class="el-table__body"]/tbody/tr[1]/td[5]/div/span').text
        # 判断预期结果与实际结果是否一致
        self.assertEqual(span, '正常', '编辑PC账户失败')
        time.sleep(1)
        # 点击清除
        self.dr.find_element_by_xpath('//button[@title="清除所有条件"]').click()
        time.sleep(2)
