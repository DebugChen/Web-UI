...
class MyTestCase_CLJK_PCZHGL_Look(unittest.TestCase):
    '''车辆监控平台-PC账户管理模块-查看测试集'''
    ...
    @BeautifulReport.add_test_img('test_CLJK_PCZHGL_Look')
    def test_CLJK_PCZHGL_Look(self):
        '''PC账户管理模块-查看测试用例'''
        # 输入用户账户
        ...
        # 点击查看
        self.dr.find_element_by_xpath(
            '//table[@class="el-table__body"]/tbody/tr[1]/td[6]/div/button[2]').click()
        time.sleep(2)
        # 获取页面元素，保存到变量
        div = self.dr.find_element_by_xpath(
            '//*[@id="app"]/div/div[6]/div[4]/div/div[1]/span').text
        # 判断预期结果与实际结果是否一致
        self.assertEqual(div, '详情', '查看PC账户失败')
        time.sleep(1)
        # 点击关闭
        self.dr.find_element_by_xpath(
            '//div[@aria-label="详情"]/div[2]/div[8]/div/button').click()
        time.sleep(2)
        # 关闭PC账户管理界面
...
if __name__ == '__main__':
    ...
    # 添加测试用例
    testunit.addTest(MyTestCase_CLJK_PCZHGL_Look('test_CLJK_PCZHGL_Look'))
    # 定义测试报告
    runner = BeautifulReport(testunit)
    runner.report(filename='车辆监控平台-PC账户管理模块-查看测试报告',
