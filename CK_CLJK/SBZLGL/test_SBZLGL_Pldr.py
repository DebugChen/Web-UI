...
class MyTestCase_CLJK_SBZLGL_Pldr(unittest.TestCase):
    '''车辆监控平台-设备资料管理模块-批量导入测试集'''
    ...
    @BeautifulReport.add_test_img('test_CLJK_SBZLGL_Pldr')
    def test_CLJK_SBZLGL_Pldr(self):
        '''设备资料管理模块-批量导入测试用例'''
        # 点击批量导入
        ...
        time.sleep(2)
        # 选取要上传的文件
        MyTestCase_CLJK_SBZLGL_Pldr.upload_file(
...
if __name__=='__main__':
    ...
    # 添加测试用例
    testunit.addTest(MyTestCase_CLJK_SBZLGL_Pldr('test_CLJK_SBZLGL_Pldr'))
    # 定义测试报告
    runner = BeautifulReport(testunit)
    runner.report(filename='车辆监控平台-设备资料管理模块-批量导入测试报告',
