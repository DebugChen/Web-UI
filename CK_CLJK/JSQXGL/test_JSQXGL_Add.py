...
class MyTestCase_CLJK_JSQXGL_Add(unittest.TestCase):
    ...
    def test_CLJK_JSQXGL_Add1(self):
        ...
        # 输入角色名称
        self.dr.find_element_by_xpath(
            '//input[@placeholder="请输入角色名称，不可包含数字和特殊字符"]').send_keys('测试角色4')
        time.sleep(1)
        # 默认正常状态
        # 点击提交按钮
        self.dr.find_element_by_xpath(
        ...
        # 输入角色名称
        self.dr.find_element_by_xpath(
            '//input[@placeholder="请输入需要搜索的角色名称"]').send_keys('测试角色4')
        time.sleep(1)
        # 点击查询
        ...
        # 判断预期结果与实际结果是否一致
        self.assertEqual(div1, '测试角色4', '新增正常角色失败')
        time.sleep(1)
        # 获取页面元素，保存到变量
    ...
    def test_CLJK_JSQXGL_Add2(self):
        ...
        # 输入角色名称
        self.dr.find_element_by_xpath(
            '//input[@placeholder="请输入角色名称，不可包含数字和特殊字符"]').send_keys('测试角色5')
        time.sleep(1)
        # 选择角色状态
        self.dr.find_element_by_xpath(
        ...
        # 输入角色名称
        self.dr.find_element_by_xpath(
            '//input[@placeholder="请输入需要搜索的角色名称"]').send_keys('测试角色5')
        time.sleep(1)
        # 点击查询
        self.dr.find_element_by_xpath('//button[@title="搜索"]').click()
        time.sleep(2)
        ...
        div = self.dr.find_element_by_xpath(
            '//table[@class="el-table__body"]/tbody/tr[1]/td[1]/div').text
        # 判断预期结果与实际结果是否一致
        self.assertEqual(div, '测试角色5', '新增锁定角色失败')
        time.sleep(1)
        # 获取页面元素，保存到变量
        div2 = self.dr.find_element_by_xpath(
            '//table[@class="el-table__body"]/tbody/tr[1]/td[2]/div').text
