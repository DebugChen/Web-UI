...
class MyTestCase_CLJK_JSQXGL_Forbidden(unittest.TestCase):
    ...
    @BeautifulReport.add_test_img('test_CLJK_JSQXGL_Forbidden1')
    def test_CLJK_JSQXGL_Forbidden1(self):
        '''角色权限管理模块-禁用测试用例-禁用'''
        # 输入角色名称
        ...
        # 点击禁用
        self.dr.find_element_by_xpath(
            '//table[@class="el-table__body"]/tbody/tr[1]/td[3]/div/button[3]').click()
        time.sleep(2)
        # 点击确认
        self.dr.find_element_by_xpath(
        ...
        # 判断预期结果与实际结果是否一致
        self.assertEqual(span, '禁用', '禁用角色失败')
        time.sleep(1)
        # 点击清除
    ...
    # @unittest.skip('跳过用例')
    # 装饰器，当你没有报错也要截图的话，那么你需要在用例里面调用save_img('001')方法
    @BeautifulReport.add_test_img('test_CLJK_JSQXGL_Forbidden2')
    def test_CLJK_JSQXGL_Forbidden2(self):
        '''角色权限管理模块-禁用测试用例-解除禁用'''
        # 输入角色名称
        self.dr.find_element_by_xpath(
            '//input[@placeholder="请输入需要搜索的角色名称"]').send_keys('测试角色02')
        time.sleep(1)
        ...
        span = self.dr.find_element_by_xpath(
            '//table[@class="el-table__body"]/tbody/tr[1]/td[2]/div/span').text
        # 判断预期结果与实际结果是否一致
        self.assertEqual(span, '正常', '解除禁用角色失败')
        time.sleep(2)
        # 点击清除
        self.dr.find_element_by_xpath('//button[@title="清空搜索条件"]').click()
        time.sleep(2)
...
if __name__ == '__main__':
    ...
    # 添加测试用例
    testunit.addTest(MyTestCase_CLJK_JSQXGL_Forbidden('test_CLJK_JSQXGL_Forbidden1'))
    testunit.addTest(MyTestCase_CLJK_JSQXGL_Forbidden('test_CLJK_JSQXGL_Forbidden2'))
    # 定义测试报告
