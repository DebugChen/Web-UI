...
class MyTestCase_CLJK_APPZHGL_Edit(unittest.TestCase):
    ...
    def tearDownClass(cls):
        ...
        #     'C:\\Users\starlinkware\PycharmProjects\\CK_CLJK\SBJK\image\APPZHGL_Cx.png')
        # 关闭浏览器
        cls.dr.quit()
    ...
    def test_CLJK_APPZHGL_Edit1(self):
        ...
        self.dr.find_element_by_xpath(
            '//input[@placeholder="请输入IMEI"]').send_keys('测试客户AA')
        time.sleep(1)
        # 点击查询
        ...
        # 判断预期结果与实际结果是否一致
        self.assertEqual(div, '13444444442', '编辑APP账户失败')
        time.sleep(1)
        # 获取页面元素，保存到变量
        span = self.dr.find_element_by_xpath(
            '//table[@class="el-table__body"]/tbody/tr[1]/td[3]/div/span').text
        # 判断预期结果与实际结果是否一致
        self.assertEqual(span, '发送安装信息', '编辑APP账户失败')
        time.sleep(1)
        # 点击清除
    ...
    def test_CLJK_APPZHGL_Edit2(self):
        ...
        self.dr.find_element_by_xpath('//form[@class="el-form"]/div[2]/div/div/input').clear()
        self.dr.find_element_by_xpath(
            '//form[@class="el-form"]/div[2]/div/div/input').send_keys('13636056931')
        time.sleep(1)
        # 选择是否发送安装信息：不发送
        self.dr.find_element_by_xpath('//form[@class="el-form"]/div[3]/div/div/div/input').click()
        self.dr.find_element_by_xpath(
        ...
        div = self.dr.find_element_by_xpath(
            '//table[@class="el-table__body"]/tbody/tr[1]/td[2]/div').text
        # 判断预期结果与实际结果是否一致
        self.assertEqual(div, '13636056931', '编辑APP账户失败')
        time.sleep(1)
        # 获取页面元素，保存到变量
        span = self.dr.find_element_by_xpath(
            '//table[@class="el-table__body"]/tbody/tr[1]/td[3]/div/span').text
        # 判断预期结果与实际结果是否一致
        self.assertEqual(span, '不发送安装信息', '编辑APP账户失败')
        time.sleep(1)
        # 点击清除
        self.dr.find_element_by_xpath('//button[@title="清除所有条件"]').click()
        time.sleep(2)
