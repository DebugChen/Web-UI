...
class MyTestCase_CLJK_AZTJFX_Qchc(unittest.TestCase):
    '''车辆监控平台-安装统计分析模块-清除缓存测试集'''
    ...
    @BeautifulReport.add_test_img('test_CLJK_AZTJFX_Qchc')
    def test_CLJK_AZTJFX_Qchc(self):
        '''安装统计分析模块-清除缓存测试用例'''
        # 选择时间
        ...
        time.sleep(3)
        # 点击清除缓存
        self.dr.find_element_by_xpath('//*[@id="app"]/div/div[6]/div[1]/div[2]/button[2]').click()
        time.sleep(5)
...
if __name__ == '__main__':
    ...
    # 添加测试用例
    testunit.addTest(MyTestCase_CLJK_AZTJFX_Qchc('test_CLJK_AZTJFX_Qchc'))
    # 定义测试报告
    runner = BeautifulReport(testunit)
    runner.report(filename='车辆监控平台-安装统计分析模块-清除缓存测试报告',
