...
class MyTestCase_CLJK_LCTJFX_Pldc(unittest.TestCase):
    '''车辆监控平台-里程统计分析模块-批量导出测试集'''
    ...
    @BeautifulReport.add_test_img('test_CLJK_LCTJFX_Pldc')
    def test_CLJK_LCTJFX_Pldc(self):
        '''里程统计分析模块-批量导出测试用例'''
        # 选择时间
        ...
        # 输入客户名称
        self.dr.find_element_by_xpath(
            '//input[@placeholder="请输入需要搜索的客户名称"]').send_keys('三大队-新华路')
        time.sleep(1)
        # 点击查询
        self.dr.find_element_by_xpath('//button[@title="搜索"]').click()
        time.sleep(5)
...
if __name__ == '__main__':
    ...
    # 添加测试用例
    testunit.addTest(MyTestCase_CLJK_LCTJFX_Pldc('test_CLJK_LCTJFX_Pldc'))
    # 定义测试报告
    runner = BeautifulReport(testunit)
    runner.report(filename='车辆监控平台-里程统计分析模块-批量导出测试报告',
