...
class MyTestCase_CLJK_LCTJFX_Ckrlc(unittest.TestCase):
    '''车辆监控平台-里程统计分析模块-测试集'''
    ...
    @BeautifulReport.add_test_img('test_CLJK_LCTJFX_Ckrlc')
    def test_CLJK_LCTJFX_Ckrlc(self):
        '''里程统计分析模块-查看日里程测试用例'''
        # 选择时间
        ...
        # 点击查看日里程
        self.dr.find_element_by_xpath(
            '//table[@class="el-table__body"]/tbody/tr/td[5]/div/button').click()
        time.sleep(2)
        # 获取页面元素，保存到变量
        span = self.dr.find_element_by_xpath(
            '//*[@id="app"]/div/div[6]/div[2]/div/div[1]/span').text
        # 判断预期结果与实际结果是否一致
        self.assertEqual(span, '日里程分析', '查看日里程失败')
        time.sleep(1)
        # 点击关闭
        self.dr.find_element_by_xpath('//div[@aria-label="日里程分析"]/div[1]/button').click()
        time.sleep(2)
...
if __name__ == '__main__':
    ...
    # 添加测试用例
    testunit.addTest(MyTestCase_CLJK_LCTJFX_Ckrlc('test_CLJK_LCTJFX_Ckrlc'))
    # 定义测试报告
    runner = BeautifulReport(testunit)
    runner.report(filename='车辆监控平台-里程统计分析模块-查看日里程测试报告',
