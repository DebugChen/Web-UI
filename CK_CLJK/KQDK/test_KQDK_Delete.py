...
class MyTestCase_CLJK_KQDK_Delete(unittest.TestCase):
    '''车辆监控平台-考勤打卡模块-删除考勤测试集'''
    ...
    @BeautifulReport.add_test_img('test_CLJK_KQDK_Delete')
    def test_CLJK_KQDK_Delete(self):
        '''考勤打卡模块-删除考勤测试用例'''
        # 点击编辑考勤
        ...
        target = self.dr.find_element_by_xpath(
            '//*[@id="app"]/div/div[6]/div[2]/div[1]/div/div[1]/div/ul/li[2]/div[2]/div[5]/button[2]')
        time.sleep(1)
        # 拖动到可见的元素去
        ...
        # 点击删除考勤
        self.dr.find_element_by_xpath(
            '//*[@id="app"]/div/div[6]/div[2]/div[1]/div/div[1]/div/ul/li[2]/div[2]/div[5]/button[2]').click()
        time.sleep(1)
        # 获取页面元素，保存到变量
        p = self.dr.find_element_by_xpath('//p[@class="el-message__content"]').text
        # 判断预期结果与实际结果是否一致
        self.assertEqual(p, '操作成功', '删除考勤失败')
        time.sleep(2)
...
if __name__ == '__main__':
    ...
    # 添加测试用例
    testunit.addTest(MyTestCase_CLJK_KQDK_Delete('test_CLJK_KQDK_Delete'))
    # 定义测试报告
    runner = BeautifulReport(testunit)
    runner.report(filename='车辆监控平台-考勤打卡模块-删除考勤测试报告',
