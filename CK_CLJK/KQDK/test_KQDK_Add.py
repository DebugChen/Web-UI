...
class MyTestCase_CLJK_KQDK_Add(unittest.TestCase):
    '''车辆监控平台-考勤打卡模块-新增考勤测试集'''
    ...
    @BeautifulReport.add_test_img('test_CLJK_KQDK_Add')
    def test_CLJK_KQDK_Add(self):
        '''考勤打卡模块-新增考勤测试用例'''
        # 点击新增考勤
        self.dr.find_element_by_xpath(
            '//*[@id="app"]/div/div[6]/div[2]/div[2]/button').click()
        time.sleep(1)
        # 输入考勤名称
        self.dr.find_element_by_xpath(
            '//input[@placeholder="请输入考勤名称"]').send_keys('测试考勤12-9-1')
        time.sleep(1)
        # 选择考勤对象（车辆监控中心）
        ...
        self.dr.find_element_by_xpath(
            ...
            '/ul/li[1]/div[4]/div[2]/button[1]').click()
        time.sleep(3)
        # 绘制护栏位置
        ...
        target = self.dr.find_element_by_xpath(
            '//*[@id="app"]/div/div[6]/div[2]/div[1]/div/div[1]/div/ul/li[1]/div[5]/button')
        time.sleep(1)
        # 拖动到可见的元素去
        ...
        # 点击保存
        self.dr.find_element_by_xpath(
            '//*[@id="app"]/div/div[6]/div[2]/div[1]/div/div[1]/div/ul/li[1]/div[5]/button').click()
        time.sleep(1)
        # 获取页面元素，保存到变量
        p = self.dr.find_element_by_xpath('//p[@class="el-message__content"]').text
        # 判断预期结果与实际结果是否一致
        self.assertEqual(p, '提交成功', '新增考勤失败')
        time.sleep(1)
...
if __name__ == '__main__':
    ...
    # 添加测试用例
    testunit.addTest(MyTestCase_CLJK_KQDK_Add('test_CLJK_KQDK_Add'))
    # 定义测试报告
    runner = BeautifulReport(testunit)
    runner.report(filename='车辆监控平台-考勤打卡模块-新增考勤测试报告',
