...
class MyTestCase_CLJK_KHXXGL_Edit(unittest.TestCase):
    ...
    @BeautifulReport.add_test_img('test_CLJK_KHXXGL_Edit1')
    def test_CLJK_KHXXGL_Edit1(self):
        '''客户资料管理模块-编辑测试用例-状态：锁定'''
        # 输入客户信息
        ...
        self.dr.find_element_by_xpath(
            '//input[@placeholder="请输入联系人"]').send_keys('陈凯测试A')
        time.sleep(2)
        # 修改联系电话
        ...
        self.dr.find_element_by_xpath(
            '//input[@placeholder="请输入安装账号"]').send_keys('陈凯测试A')
        time.sleep(2)
        # 选择状态：锁定
        self.dr.find_element_by_xpath(
            '//form[@class="el-form"]/div[6]/div/label[2]/span[1]/span').click()
        time.sleep(1)
        # 点击提交按钮
        self.dr.find_element_by_xpath(
    ...
    # @unittest.skip('跳过用例')
    # 装饰器，当你没有报错也要截图的话，那么你需要在用例里面调用save_img('001')方法
    @BeautifulReport.add_test_img('test_CLJK_KHXXGL_Edit2')
    def test_CLJK_KHXXGL_Edit2(self):
        '''客户资料管理模块-编辑测试用例-状态：正常'''
        # 输入客户信息
        self.dr.find_element_by_xpath(
            '//input[@placeholder="请输入需要搜索的客户名称"]').send_keys('陈凯测试A')
        time.sleep(2)
        # 点击查询按钮
        self.dr.find_element_by_xpath('//button[@title="搜索"]').click()
        time.sleep(2)
        ...
        self.dr.find_element_by_xpath('//input[@placeholder="请输入联系人"]').clear()
        self.dr.find_element_by_xpath(
            '//input[@placeholder="请输入联系人"]').send_keys('陈凯测试')
        time.sleep(2)
        # 修改联系电话
        self.dr.find_element_by_xpath('//input[@placeholder="请输入联系电话"]').clear()
        self.dr.find_element_by_xpath(
            '//input[@placeholder="请输入联系电话"]').send_keys('13365611111')
        time.sleep(2)
        # 修改地址
        self.dr.find_element_by_xpath(
            '//input[@placeholder="请输入联系地址，不超过50个字符"]').clear()
        ...
        self.dr.find_element_by_xpath('//input[@placeholder="请输入安装账号"]').clear()
        self.dr.find_element_by_xpath(
            '//input[@placeholder="请输入安装账号"]').send_keys('陈凯测试')
        time.sleep(2)
        # 选择状态：正常
        self.dr.find_element_by_xpath(
        ...
        div = self.dr.find_element_by_xpath(
            '//table[@class="el-table__body"]/tbody/tr[1]/td[1]/div').text
        # 判断预期结果与实际结果是否一致
        self.assertEqual(div, '陈凯测试', '编辑客户失败')
        time.sleep(1)
        # 定义变量，用于存放获取到的页面元素
        span = self.dr.find_element_by_xpath(
            '//table[@class="el-table__body"]/tbody/tr[1]/td[6]/div/span').text
        # 判断预期结果与实际结果是否一致
        self.assertEqual(span, '正常', '编辑客户失败')
        time.sleep(2)
        # 点击清除查询条件
        self.dr.find_element_by_xpath('//button[@title="清空搜索条件"]').click()
        time.sleep(2)
...
if __name__=='__main__':
    ...
    # 添加测试用例
    testunit.addTest(MyTestCase_CLJK_KHXXGL_Edit('test_CLJK_KHXXGL_Edit1'))
    testunit.addTest(MyTestCase_CLJK_KHXXGL_Edit('test_CLJK_KHXXGL_Edit2'))
    # 定义测试报告
