...
class MyTestCase_CLJK_JSQXGL_Forbidden(unittest.TestCase):
    '''车辆监控平台-角色权限管理模块-禁用测试集'''
    ...
    @BeautifulReport.add_test_img('test_CLJK_JSQXGL_Forbidden')
    def test_CLJK_JSQXGL_Forbidden(self):
        '''角色权限管理模块-禁用测试用例'''
        # 输入角色名称
        self.dr.find_element_by_xpath(
            '//input[@placeholder="请输入需要搜索的角色名称"]').send_keys('荆州项目')
        time.sleep(1)
        # 点击查询
        ...
        # 点击禁用
        self.dr.find_element_by_xpath(
            '//table[@class="el-table__body"]/tbody/tr[HT_XBDY]/td[3]/div/button[3]').click()
        time.sleep(2)
        # 点击确认
        self.dr.find_element_by_xpath(
            '//body[@class="el-popup-parent--hidden"]/div[3]/div/div[3]/button[2]').click()
        time.sleep(2)
        # 获取页面元素，保存到变量
        div1 = self.dr.find_element_by_xpath(
            '//table[@class="el-table__body"]/tbody/tr[HT_XBDY]/td[2]/div').text
        # 判断预期结果与实际结果是否一致
        self.assertEqual(div1, '禁用', '禁用角色失败')
        time.sleep(4)
        # 点击清除
...
if __name__ == '__main__':
    ...
    # 添加测试用例
    testunit.addTest(MyTestCase_CLJK_JSQXGL_Forbidden('test_CLJK_JSQXGL_Forbidden'))
    # 定义测试报告
    runner = BeautifulReport(testunit)
    runner.report(filename='车辆监控平台-角色权限管理模块-禁用测试报告',
