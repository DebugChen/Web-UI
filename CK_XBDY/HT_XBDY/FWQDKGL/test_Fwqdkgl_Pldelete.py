...
class MyTestCase_XBDY_FWQDKGL_PlDelete(MyTestCase_XBDY_Login_GG):
    '''新北斗云平台-服务器端口管理模块-批量删除测试集'''
    ...
    @BeautifulReport.add_test_img('test_XBDY_FWQDKGL_PlDelete')
    def test_XBDY_FWQDKGL_PlDelete(self):
        '''服务器端口管理模块-批量删除测试用例'''
        ...
        time.sleep(1)
        # 点击批量删除
        self.dr.find_element_by_xpath('//a[@class="right"]').click()
        time.sleep(2)
        # 点击确认
        self.dr.find_element_by_xpath(
        ...
        # 判断预期结果与实际结果是否一致
        self.assertEqual(span, '删除成功', '批量删除服务器端口失败')
        time.sleep(1)
...
if __name__ == '__main__':
    ...
    # 添加测试用例
    testunit.addTest(MyTestCase_XBDY_FWQDKGL_PlDelete('test_XBDY_FWQDKGL_PlDelete'))
    # 定义测试报告
    runner = BeautifulReport(testunit)
    runner.report(filename='新北斗云平台-服务器端口管理模块-批量删除测试报告',
