...
class MyTestCase_XBDY_Sbjrgl_Pldc(MyTestCase_XBDY_Login_GG):
    '''新北斗云平台-设备接入管理模块-批量导出设备测试集'''
    ...
    @BeautifulReport.add_test_img('test_XBDY_Sbjrgl_Pldc')
    def test_XBDY_Sbjrgl_Pldc(self):
        '''设备接入管理模块-批量导出设备测试用例'''
        ...
        self.dr.find_element_by_xpath(
            '//tbody[@class="ivu-table-tbody"]/tr[3]/td[1]/div/label/span/input').click()
        time.sleep(1)
        # 点击批量导出
        self.dr.find_element_by_xpath('//div[@class="search-show"]/button[5]').click()
        time.sleep(1)
        # 获取页面元素，保存到变量
...
if __name__ == '__main__':
    ...
    # 添加测试用例
    testunit.addTest(MyTestCase_XBDY_Sbjrgl_Pldc('test_XBDY_Sbjrgl_Pldc'))
    # 定义测试报告
    runner = BeautifulReport(testunit)
    runner.report(filename='新北斗云平台-设备接入管理模块-批量导出测试报告',
