...
class MyTestCase_XBDY_Sbjrgl_PlDelete(MyTestCase_XBDY_Login_GG):
    '''新北斗云平台-设备接入管理模块-批量删除测试集'''
    ...
    @BeautifulReport.add_test_img('test_XBDY_Sbjrgl_PlDelete')
    def test_XBDY_Sbjrgl_PlDelete(self):
        '''设备接入管理模块-批量删除测试用例'''
        ...
        # 勾选需要批量删除设备
        self.dr.find_element_by_xpath(
            '//tbody[@class="ivu-table-tbody"]/tr/td[1]/div/label/span/input').click()
        time.sleep(1)
        # 点击批量删除
        self.dr.find_element_by_xpath('//div[@class="search-show"]/button[6]').click()
        time.sleep(1)
        # # 点击确定
        # self.dr.find_element_by_xpath(
        ...
        # 判断预期结果与实际结果是否一致
        self.assertEqual(span, '成功删除', '批量删除设备失败')
        time.sleep(1)
...
if __name__ == '__main__':
    ...
    # 添加测试用例
    testunit.addTest(MyTestCase_XBDY_Sbjrgl_PlDelete('test_XBDY_Sbjrgl_PlDelete'))
    # 定义测试报告
    runner = BeautifulReport(testunit)
    runner.report(filename='新北斗云平台-设备接入管理模块-批量删除测试报告',
