...
class MyTestCase_XBDY_Sbjrgl_Edit(MyTestCase_XBDY_Login_GG):
    '''新北斗云平台-设备接入管理模块-编辑设备测试集'''
    ...
    @BeautifulReport.add_test_img('test_XBDY_Sbjrgl_Edit1')
    def test_XBDY_Sbjrgl_Edit1(self):
        '''设备接入管理模块-编辑设备测试用例-编辑后'''
        ...
        time.sleep(1)
        self.dr.find_element_by_xpath(
            '//input[@placeholder="请输入IMEI号"]').send_keys('861097040963070')
        time.sleep(1)
        # 点击确定
        self.dr.find_element_by_xpath(
        ...
        self.dr.find_element_by_xpath(
            '//div[@class="ivu-modal-wrap vertical-center-modal"]/div'
            '/div/div[2]/div/form/div[2]/div[2]/div/div/div/div[1]/div').click()
        time.sleep(1)
        # 选中：C2608
        self.dr.find_element_by_xpath(
            '//div[@class="ivu-select ivu-select-visible ivu-select-single'
        ...
        # 判断预期结果与实际结果是否一致
        self.assertEqual(span, 'C2608', '编辑设备失败')
        time.sleep(1)
        # 点击清除
        self.dr.find_element_by_xpath('//div[@class="search-show"]/button[2]').click()
        time.sleep(3)
    ...
    # @unittest.skip('跳过用例')
    # 装饰器，当你没有报错也要截图的话，那么你需要在用例里面调用save_img('001')方法
    @BeautifulReport.add_test_img('test_XBDY_Sbjrgl_Edit2')
    def test_XBDY_Sbjrgl_Edit2(self):
        '''设备接入管理模块-编辑设备测试用例-编辑前'''
        # 输入IMEi
        self.dr.find_element_by_xpath(
            '//div[@class="search-show"]/div[1]/div/div[1]/button').click()
        time.sleep(1)
        ...
        self.dr.find_element_by_xpath(
            '//div[@class="ivu-modal-wrap vertical-center-modal"]/div'
            '/div/div[2]/div/form/div[1]/div[2]/div/div/div/div[1]/div').click()
        time.sleep(1)
        # 选中：公网
        self.dr.find_element_by_xpath(
            '//div[@class="ivu-select ivu-select-visible ivu-select-single'
        ...
        self.dr.find_element_by_xpath(
            '//div[@class="ivu-select ivu-select-visible ivu-select-single'
            ' ivu-select-default"]/div[2]/ul[2]/li[4]').click()
        time.sleep(1)
        # 点击确定
        self.dr.find_element_by_xpath(
            '//div[@class="ivu-modal-wrap vertical-center-modal"]'
        ...
        span = self.dr.find_element_by_xpath(
            '//tbody[@class="ivu-table-tbody"]/tr[1]/td[3]/div/span').text
        # 判断预期结果与实际结果是否一致
        self.assertEqual(span, 'C2612', '编辑设备失败')
        time.sleep(1)
        # 点击清除
        self.dr.find_element_by_xpath('//div[@class="search-show"]/button[2]').click()
        time.sleep(3)
...
if __name__ == '__main__':
    ...
    # 添加测试用例
    testunit.addTest(MyTestCase_XBDY_Sbjrgl_Edit('test_XBDY_Sbjrgl_Edit1'))
    testunit.addTest(MyTestCase_XBDY_Sbjrgl_Edit('test_XBDY_Sbjrgl_Edit2'))
    # 定义测试报告
    runner = BeautifulReport(testunit)
    runner.report(filename='新北斗云平台-设备接入管理模块-编辑测试报告',
