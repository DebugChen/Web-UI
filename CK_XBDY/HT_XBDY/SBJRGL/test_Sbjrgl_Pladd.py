...
class MyTestCase_XBDY_Sbjrgl_Pladd(MyTestCase_XBDY_Login_GG):
    '''新北斗云平台-设备接入管理模块-批量接入测试集'''
    ...
    @BeautifulReport.add_test_img('test_XBDY_Sbjrgl_Pladd')
    def test_XBDY_Sbjrgl_Pladd(self):
        '''设备接入管理模块-批量接入测试用例'''
        ...
        # 点击批量接入
        self.dr.find_element_by_xpath('//div[@class="search-show"]/button[4]').click()
        time.sleep(1)
        # 点击选取文件弹出上传文件窗口
        ...
        time.sleep(3)
        # 选取要上传的文件
        MyTestCase_XBDY_Sbjrgl_Pladd.upload_file(
            r'E:\123.exe ', 'chrome', r'E:\自动化下载文件存放\新北斗云_批量接入设备模板.xlsx')
        time.sleep(2)
        # 点击上传
        ...
        # 判断预期结果与实际结果是否一致
        self.assertEqual(span, '数据导入成功！', '批量接入失败')
        time.sleep(1)
        # 点击关闭
        self.dr.find_element_by_xpath(
            '//div[@class="ivu-modal-wrap vertical-center-modal"]'
...
if __name__ == '__main__':
    ...
    # 添加测试用例
    testunit.addTest(MyTestCase_XBDY_Sbjrgl_Pladd('test_XBDY_Sbjrgl_Pladd'))
    # 定义测试报告
    runner = BeautifulReport(testunit)
    runner.report(filename='新北斗云平台-设备接入管理模块-批量接入测试报告',
