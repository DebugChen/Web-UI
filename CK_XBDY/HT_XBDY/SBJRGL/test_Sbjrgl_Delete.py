...
class MyTestCase_XBDY_Sbjrgl_Delete(MyTestCase_XBDY_Login_GG):
    '''新北斗云平台-设备接入管理模块-删除测试集'''
    ...
    @BeautifulReport.add_test_img('test_XBDY_Sbjrgl_Delete')
    def test_XBDY_Sbjrgl_Delete(self):
        '''设备接入管理模块-删除测试用例'''
        ...
        time.sleep(2)
        # 点击删除
        self.dr.find_element_by_xpath('//div[@class="ivu-table-cell-slot"]/button[2]').click()
        time.sleep(1)
        # 点击确定
        self.dr.find_element_by_xpath(
        ...
        # 判断预期结果与实际结果是否一致
        self.assertEqual(span, '成功删除', '删除设备失败')
        time.sleep(1)
...
if __name__ == '__main__':
    ...
    # 添加测试用例
    testunit.addTest(MyTestCase_XBDY_Sbjrgl_Delete('test_XBDY_Sbjrgl_Delete'))
    # 定义测试报告
    runner = BeautifulReport(testunit)
    runner.report(filename='新北斗云平台-设备接入管理模块-删除测试报告',
