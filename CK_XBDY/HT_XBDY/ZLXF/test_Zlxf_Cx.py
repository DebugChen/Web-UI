...
class MyTestCase_XBDY_Zlxf_Cx(MyTestCase_XBDY_Login_GG):
    '''新北斗云平台-指令下发模块-查询测试集'''
    ...
    @BeautifulReport.add_test_img('test_XBDY_Zlxf_Cx')
    def test_XBDY_Zlxf_Cx(self):
        '''指令下发模块-查询测试用例'''
    ...
    # @unittest.skip('跳过用例')
    # 装饰器，当你没有报错也要截图的话，那么你需要在用例里面调用save_img('001')方法
    @BeautifulReport.add_test_img('test_XBDY_Zlxf_Fy')
    def test_XBDY_Zlxf_Fy(self):
        '''指令下发模块-分页测试用例'''
        # #按50条/页
        # 点击分页显示
        self.dr.find_element_by_xpath(
...
if __name__ == '__main__':
    ...
    # 添加测试用例
    testunit.addTest(MyTestCase_XBDY_Zlxf_Cx('test_XBDY_Sblb_Cx'))
    testunit.addTest(MyTestCase_XBDY_Zlxf_Cx('test_XBDY_Sblb_Fy'))
    # 定义测试报告
    runner = BeautifulReport(testunit)
    runner.report(filename='新北斗云平台-指令下发模块-查询测试报告',
