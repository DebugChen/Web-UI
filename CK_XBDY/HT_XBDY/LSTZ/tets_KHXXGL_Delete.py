...
class MyTestCase_CLJK_KHXXGL_Delete(unittest.TestCase):
    ...
    def tearDownClass(cls):
        ...
        #     'C:\\Users\starlinkware\PycharmProjects\\CK_CLJK\LSTZ\image1\KHXXGL_Delete.png')
        # 关闭浏览器
        cls.dr.quit()

    # @unittest.skip('跳过用例')
    # 装饰器，当你没有报错也要截图的话，那么你需要在用例里面调用save_img('001')方法
    @BeautifulReport.add_test_img('test_CLJK_KHXXGL_Delete')
    def test_CLJK_KHXXGL_Delete(self):
        '''客户信息管理模块-删除测试用例'''
        # 查询需要删除的客户信息
        self.dr.find_element_by_xpath(
            '//input[@placeholder="请输入需要搜索的客户名称"]').send_keys('随州')
        time.sleep(1)
        # 点击搜索按钮
        ...
        # 判断预期结果与实际结果是否一致
        self.assertEqual(span, '共 0 条', '删除客户失败')
        time.sleep(2)
...
if __name__=='__main__':
    ...
    # 添加测试用例
    testunit.addTest(MyTestCase_CLJK_KHXXGL_Delete('test_CLJK_KHXXGL_Delete'))
    # 定义测试报告
