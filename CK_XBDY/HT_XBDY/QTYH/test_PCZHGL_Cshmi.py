...
class MyTestCase_CLJK_PCZHGL_Cshmi(unittest.TestCase):
    '''车辆监控平台-PC账户管理模块-初始化密码测试集'''
    ...
    @BeautifulReport.add_test_img('test_CLJK_PCZHGL_Cshmi')
    def test_CLJK_PCZHGL_Cshmi(self):
        '''PC账户管理模块-初始化密码测试用例'''
        # 输入用户账户
        self.dr.find_element_by_xpath(
            '//input[@placeholder="请输入需要搜索的用户账号"]').send_keys('liuchunl')
        time.sleep(1)
        # 点击查询
        ...
        # 点击初始化密码
        self.dr.find_element_by_xpath(
            '//table[@class="el-table__body"]/tbody/tr/td[6]/div/button[HT_XBDY]').click()
        time.sleep(2)
        # 点击确定
        self.dr.find_element_by_xpath(
        ...
        # 判断预期结果与实际结果是否一致
        self.assertEqual(p, '初始化密码成功', '初始化密码失败')
        time.sleep(2)
...
if __name__ == '__main__':
    ...
    # 添加测试用例
    testunit.addTest(MyTestCase_CLJK_PCZHGL_Cshmi('test_CLJK_PCZHGL_Cshmi'))
    # 定义测试报告
    runner = BeautifulReport(testunit)
    runner.report(filename='车辆监控平台-PC账户管理模块-初始化密码测试报告',
