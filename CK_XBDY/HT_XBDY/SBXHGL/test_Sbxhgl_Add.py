...
class MyTestCase_XBDY_SBXHGL_Add(MyTestCase_XBDY_Login_GG):
    '''新北斗云平台-设备型号管理模块-新增测试集'''
    ...
    @BeautifulReport.add_test_img('test_XBDY_SBXHGL_Add')
    def test_XBDY_SBXHGL_Add(self):
        '''设备型号管理模块-新增测试用例'''
        ...
        # 输入供应商设备型号
        self.dr.find_element_by_xpath(
            '//input[@placeholder="请输入供应商设备型号"]').send_keys('测试型号S004')
        time.sleep(1)
        # 输入公司设备型号
        self.dr.find_element_by_xpath(
            '//input[@placeholder="请输入公司设备型号"]').send_keys('测试型号S004')
        time.sleep(1)
        # 输入接入协议
        self.dr.find_element_by_xpath(
            '//input[@placeholder="请输入接入协议"]').send_keys('101')
        time.sleep(1)
        # 点击确认
        ...
        # 输入搜索内容
        self.dr.find_element_by_xpath(
            '//input[@placeholder="搜索内容"]').send_keys('测试型号S004')
        time.sleep(1)
        # 点击查询
        ...
        # 判断预期结果与实际结果是否一致
        self.assertEqual(span, '测试型号S004', '新增设备型号失败')
        time.sleep(1)
        # 点击清除
...
if __name__ == '__main__':
    ...
    # 添加测试用例
    testunit.addTest(MyTestCase_XBDY_SBXHGL_Add('test_XBDY_SBXHGL_Add'))
    # 定义测试报告
    runner = BeautifulReport(testunit)
    runner.report(filename='新北斗云平台-设备型号管理模块-新增测试报告',
