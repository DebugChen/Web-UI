...
class MyTestCase_XBDY_SBXHGL_Delete(MyTestCase_XBDY_Login_GG):
    '''新北斗云平台-设备型号管理模块-删除测试集'''
    ...
    @BeautifulReport.add_test_img('test_XBDY_SBXHGL_Delete')
    def test_XBDY_SBXHGL_Delete(self):
        '''设备型号管理模块-删除测试用例'''
        ...
        # 输入搜索内容
        self.dr.find_element_by_xpath(
            '//input[@placeholder="搜索内容"]').send_keys('测试型号S002')
        time.sleep(1)
        # 点击查询
        ...
        time.sleep(2)
        # 获取页面元素，保存到变量
        span = self.dr.find_element_by_xpath('//span[@class="ivu-page-total"]').text
        # 判断预期结果与实际结果是否一致
        self.assertEqual(span, '共 0 条', '删除设备型号失败')
        time.sleep(1)
        # 点击清除
        self.dr.find_element_by_xpath('//div[@class="operate-details"]/button[2]').click()
        time.sleep(2)
...
if __name__ == '__main__':
    ...
    # 添加测试用例
    testunit.addTest(MyTestCase_XBDY_SBXHGL_Delete('test_XBDY_SBXHGL_Delete'))
    # 定义测试报告
    runner = BeautifulReport(testunit)
    runner.report(filename='新北斗云平台-设备型号管理模块-删除测试报告',
