...
class MyTestCase_ZHHL_AZXLGL_Cxld(unittest.TestCase):
    '''智慧护栏平台-安装线路管理模块-撤销测试集'''
    ...
    @BeautifulReport.add_test_img('test_ZHHL_AZXLGL_Cxld')
    def test_ZHHL_AZXLGL_Cxld(self):
        '''安装线路管理模块-撤销测试用例'''
        # 输入路段名称
        self.dr.find_element_by_xpath(
            '//input[@placeholder="请输入需要搜索的路段名称"]').send_keys('aaaa')
        time.sleep(1)
        # 点击查询
...
if __name__ == '__main__':
    ...
    # 添加测试用例
    testunit.addTest(MyTestCase_ZHHL_AZXLGL_Cxld('test_ZHHL_AZXLGL_Cxld'))
    # 定义测试报告
    runner = BeautifulReport(testunit)
    runner.report(filename='智慧护栏平台-安装线路管理模块-撤销测试报告',
